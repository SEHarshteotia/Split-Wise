...
function FriendDetailsCollectPage() {
    ...
    const fetchTransactions = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/friends_expenses/${user_id}/collect/${friend_id}`
        );
        if (!response.ok) throw new Error("Failed to fetch transactions");
        const data = await response.json();

        if (data.length > 0) setFriendName(data[0].friend_name); // get friend's name
...
export default FriendDetailsCollectPage;
